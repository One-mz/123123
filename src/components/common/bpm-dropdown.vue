<template>
  <div class='mt-1 vux-1px-t'>
    <div @click="showPopUp">
      <x-input text-align="right" v-model="showSelected"  :title="title" @on-focus="showPopUp" :show-clear="false" readonly>
        <x-icon slot="right" type="android-arrow-dropdown" v-show="!this.$store.state.readOnly" @click="showPopUp" size="18"></x-icon>
      </x-input>
    </div>

    <div v-transfer-dom>
      <van-popup round position="bottom" v-model="popShow">
...
export default {
    ...
    // 初始化加载
      ...
      var self = this;
			...
			this.picker = []
			this.selectNodes = []
			let setVars = { vars: this.criteria }
      if(!this.criteria) {
        ...
        return httpService(this).dropDownService(setVars).then(res => {
            let data = res.body;
            self.ajaxData = data;
            if(this.ignoredItem){
