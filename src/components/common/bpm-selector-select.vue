...
import uuidMixin from 'vux/src/mixins/uuid'
import cleanStyle from 'vux/src/libs/clean-style'
			...
			validator(val) {
				if (!val.length || val.length !== 2) {
					if (process.env.NODE_ENV === 'development') {
						console.error(
							"[VUX error] selector prop:valueMap's length should be 2",
						)
					}
					return false
				}
...
@import '~vux/src/styles/weui/widget/weui_cell/weui_form/weui_select';
